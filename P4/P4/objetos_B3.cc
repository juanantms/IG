...
void _triangulos3D::calcular_normales_caras() {
  ...
  for(i=0; i<n_c; i++) {
    ...
    modulo = sqrt(normales_caras[i].x*normales_caras[i].x + normales_caras[i].y*normales_caras[i].y + normales_caras[i].z*normales_caras[i].z);
...
void _triangulos3D::colors_plano(float r, float g, float b, float pos_lx, float pos_ly, float pos_lz)
{
  int i, n_c;
  n_c = caras.size();
  ...
  for (i = 0; i < n_c; i++) {
    ...
    colores_caras[i].b = b*0.101;
    luz.x = pos_lx - vertices[caras[i]._0].x; 
    luz.y = pos_ly - vertices[caras[i]._0].y; 
    luz.z = pos_lz - vertices[caras[i]._0].z; 
    modulo = sqrt(luz.x*luz.x + luz.y*luz.y + luz.z*luz.z);
    luz.x/=modulo;
    luz.y/=modulo;
    luz.z/=modulo;
    escalar = normales_caras[i].x*luz.x + normales_caras[i].y*luz.y + normales_caras[i].z*luz.z;
    if( escalar > 0.0 ) {
...
_mcubo::_mcubo(float tam)
{
  ...
  //colors_random();
  colors_plano(0.7, 0.4, 0.1, 0, 20, 0);
}
...
void _objeto_ply::parametros(char *archivo)
{
  ...
  colors_plano(0.7, 0.5, 0.2, 0.0, 20, 0.0);
